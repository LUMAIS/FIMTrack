        ...
        Mat claheRoi;  // Image with the corrected contrast
        {
            ...
            && countMaskPix(maskBg, 0xFF, LARVA_TRUAREA_HARDMIN) >= LARVA_TRUAREA_HARDMIN) {  // Half x Half CLAHE clip limit
                if(DEV_MODE >= 3 && extraVis) {
                    ...
                    maskRoi.setTo(cv::GC_PR_BGD);  // Restore maskRoi
                }
                // Erode the original background mask to avoid holes in dark larvae and prevent loss of brightness values (see vid_1-013, 15, 31)
                ...
                cv::morphologyEx(maskAth, maskAth, cv::MORPH_OPEN, erdKern, Point(-1, -1), 1);  // MORPH_OPEN, MORPH_CLOSE
                //if(DEV_MODE >= 10 && extraVis)
...
//              cv::morphologyEx(maskTmp, maskTmp, cv::MORPH_OPEN, erdKern, Point(-1, -1), 1);  // MORPH_OPEN, MORPH_CLOSE
//                //if(DEV_MODE >= 10 && extraVis)
...
//                    showCvWnd("2.3.b0.CntRfnAthProbFgRoi", maskTmp, cvWnds);
                ...
                //Mat maskFlt;
                ////cv::erode(maskAth, maskFlt, Mat(), Point(-1, -1), 1);  // 8 .. 12 .. 16; Scalar(cv::GC_PR_FGD); 1 or
...
//                    showCvWnd("3.3.RfnEdgesOrigRoi", edgesOrig, cvWnds);
                ...
                if(DEV_MODE >= 7 && extraVis) {
                    showCvWnd("6.1.1.ProbFgClahe", maskClaheFg, cvWnds);
                    showCvWnd("6.1.2.ProbBgClahe", maskClaheBg, cvWnds);
                }

                // Refine the pure background as mutual of the both former pure backgrounds
                // Note: that is not applicable for the Foreground, which is excessive in CLAHE
                cv::bitwise_and(maskBg, maskClaheBg, maskBg);

                // Ensure that maskClaheBg is not inside the foreground area to avoid holes in larvae (see vid_1-054, 06, 40, 52), but the counterexamples are vid_1-020-25, where true background disappears => included into resulting larvae
...
//                    showCvWnd("7.3.b0.CntRfnAthProbFgClaheRoi", maskTmp, cvWnds);
                ...
                if(countMaskPix(maskClaheRoi, cv::GC_FGD, 1) && countMaskPix(maskClaheRoi, cv::GC_BGD, 1)) {
                    Mat bgdModel, fgdModel;
                    Mat imgClr;
                    cv::cvtColor(imgRoi, imgClr, cv::COLOR_GRAY2BGR);  // CV_8UC3; imgCorr
...
//                    if(DEV_MODE >= 4 && extraVis)
//                      showCvWnd("9.6.0.0.RfnEdgRoiMask", edgesOrig, cvWnds);
...
//                    cv::morphologyEx(maskTmp, maskTmp, cv::MORPH_CLOSE, erdKern, Point(-1, -1), 1);
                    ...
                    //cv::morphologyEx(edgesOrig, edgesOrig, cv::MORPH_CLOSE, erdKern, Point(-1, -1), 2);  // 1..2
...
//                    edgesOrig += maskTmp;
//                    if(DEV_MODE >= 4 && extraVis) {
//                        showCvWnd("9.6.0.2.ErdRemMaskGcProbFg", maskTmp, cvWnds);
//                        showCvWnd("9.6.0.2e.RfnErdRemMaskGcProbFg", edgesOrig, cvWnds);
//                    }

                    {
                        // Exclude (epanded) small components of combined 9.2.RfbErdCn... and 2.4 ErdCntRf... to reduce outliers
                        // maskAth is 2.4.ErdCntRfnAthProbFgRoi, maskClaheAth is 9.2.RfnErdCntRfnAthProbFgRoi
                        // Erode maskAth and maskClaheAth to separate larvae (see: vid-1_009-012, 10)
                        // ATTENTION: morpholody on the combined image != combination of the morphology on parts
                        //cv::morphologyEx(maskAth, maskTmp, cv::MORPH_OPEN, erdKern, Point(-1, -1), 1);
                        cv::erode(maskAth, maskTmp, erdKern, Point(-1, -1), 1);
                        Mat maskAthCmb;
                        cv::erode(maskClaheAth, maskAthCmb, erdKern, Point(-1, -1), 1);
                        maskAthCmb += maskTmp;
                        //Mat maskAthCmb; = maskAth + maskClaheAth;
                        //cv::bitwise_or(maskAth, maskClaheAth, maskAthCmb);
                        //cv::erode(maskAthCmb, maskAthCmb, erdKern, Point(-1, -1), 1);
                        //cv::morphologyEx(maskAthCmb, maskAthCmb, cv::MORPH_OPEN, erdKern, Point(-1, -1), 1);
                        ...
                        //// Reduce noisy contours inside larvae if possible.
                        //// Note: The external contours are bolder and affected less by the morphology.
                        //// ATTENTION: reductive morphology operations (erode, OPEN) destruct contours separating larvae
                        //cv::morphologyEx(edgesOrig, edgesOrig, cv::MORPH_CLOSE, erdKern, Point(-1, -1), 1);
                        //if(DEV_MODE >= 4 && extraVis)
                        //    showCvWnd("9.6.0.3o.RfnRdsEdgesOrigRoi", edgesOrig, cvWnds);

                        //// Exlude outliers from maskTmp, which are small components, having area < 1/4 of the median.
                        //cv::findContours(maskAthCmb, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);
                        //struct AreaInfo {
                        //    float value;
                        //    uint16_t index;
                        //};
                        ...
                        //    areas.push_back(cv::contourArea(cont));
                        //}

                        // Substract reduced CLAHE foreground 6.4
                        // Ensure that maskClaheFg do not contains holes in larva
                        cv::findContours(maskClaheFg, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);
                        Mat  maskProbClaheFg = Mat::zeros(maskClaheFg.size(), maskClaheFg.type());
                        cv::drawContours(maskProbClaheFg, contours, -1, 0xFF, cv::FILLED);  // cv::FILLED, 1

                        //// Remove noise inside larva caused by background
                        ////cv::dilate(maskProbClaheFg, maskTmp, erdKern, Point(-1, -1), 1);
                        ////cv::erode(maskTmp, maskTmp, erdKern, Point(-1, -1), 1);
                        ...
                        if(DEV_MODE >= 8 && extraVis) {
                            ...
                            showCvWnd("9.6.0.4.1.FlCntClaheFgRoi", maskProbClaheFg, cvWnds);
                        }

                        // Ensure that backgrouned is not covered with the closed CLAHE foreground contours
                        maskProbClaheFg -= maskClaheBg;  // cv::bitwise_not(maskProbClaheFg, maskClaheBg);
                        if(DEV_MODE >= 4 && extraVis)
                            showCvWnd("9.6.0.4.2.ProbClaheFgRoi", maskProbClaheFg, cvWnds);

                        // Note: reuse maskAthCmb to store approximate contours of the CLAHE foreground
                        cv::subtract(maskProbClaheFg, maskAthCmb, maskAthCmb);
                        if(DEV_MODE >= 4 && extraVis)
                            showCvWnd("9.6.0.5.AprCntClaheFgRoi", maskAthCmb, cvWnds);

//                        //// Note: counter examples for iterations=4-5, causing larvae merge: vid_1-021-023, 26-29
//                        cv::erode(maskAthCmb, maskTmp, erdKern, Point(-1, -1), 6);  // 4 .. 6;
//                        //cv::morphologyEx(maskTmp, maskTmp, cv::MORPH_OPEN, erdKern, Point(-1, -1), 2);  // MORPH_OPEN, MORPH_CLOSE
                        ...
                        if(DEV_MODE >= 4 && extraVis) {
                            showCvWnd("9.6.0.5f.FltAprCntClaheFgRoi", maskAthCmb, cvWnds);
                            showCvWnd("9.6.0.5b.BlobsCntClaheFgRoi", maskTmp, cvWnds);
                        }

                        edgesOrig -= maskTmp;
                        // Temporary store approximate contours of the CLAHE foreground reduced by the original edges
                        cv::subtract(maskAthCmb, edgesOrig, maskTmp);
                        if(DEV_MODE >= 4 && extraVis) {
                            showCvWnd("9.6.0.6.0.Rds2RfnEdgesOrigRoi", edgesOrig, cvWnds);
                            showCvWnd("9.6.0.6.1.FltAprCntClaheRdsEdgesRoi", maskTmp, cvWnds);
                        }
                        ...
                        // Extend edges with the contours of the combined mask 9.6.0.5.RfnFlCntClaheFgRoi [? that overlap with the extended edges to reduce larva splitting, which might not separate mergin larvae]
                        ...
                        if(DEV_MODE >= 4 && extraVis)
                            showCvWnd("9.6.0.6.2.CntExtRds2RfnEdgesOrigRoi", edgesOrig, cvWnds);
                        ...
                        cv::dilate(maskTmp, maskTmp, erdKern, Point(-1, -1), 1);  // Extend excluding contours to filter out inner edges more
                        maskAthCmb -= maskTmp;
                        maskProbClaheFg -= maskAthCmb;
                        if(DEV_MODE >= 4 && extraVis) {
                            showCvWnd("9.6.0.6.3.CntExtRds2RfnEdgesOrigRoi", maskAthCmb, cvWnds);
                            showCvWnd("9.6.0.6.4.RdsFlCntClaheFgRoi", maskProbClaheFg, cvWnds);
                        }
                        // Erode the reduced probable CLAHE foreground (?with closed holes)
                        //cv::morphologyEx(maskTmp, maskTmp, cv::MORPH_CLOSE, erdKern, Point(-1, -1), 2);  // MORPH_OPEN, MORPH_CLOSE
                        ...
                        if(DEV_MODE >= 4 && extraVis) {
                            showCvWnd("9.6.0.6.5.ErdRdsFlCntClaheFgRoi", maskProbClaheFg, cvWnds);
                            showCvWnd("9.6.0.6.6.RdsCntExtRds2RfnEdgesOrigRoi", edgesOrig, cvWnds);
                        }

                        // Extend edges, or close holes in edges (which are already cleaned from the internal noise) to separate merging larvae
                        cv::morphologyEx(edgesOrig, edgesOrig, cv::MORPH_CLOSE, erdKern, Point(-1, -1), 1);
                        if(DEV_MODE >= 4 && extraVis)
                            showCvWnd("9.6.0.6.7.CleanedEdgesOrigRoi", edgesOrig, cvWnds);
                    }
                    ...
                    //// Substract reduced CLAHE background 6.4 (foreground is not suitable because it may contain holes, which causes holes in larva)
                    //if(DEV_MODE >= 4 && extraVis)
                    //    showCvWnd("9.6.0.7.MaskClaheBg", maskClaheBg, cvWnds);
                    //cv::erode(maskClaheBg, maskTmp, erdKern, Point(-1, -1), 1);  // 6 .. 10;
                    //if(DEV_MODE >= 4 && extraVis)
                    //    showCvWnd("9.6.0.8.ErdMaskClaheBg", maskTmp, cvWnds);
                    ////cv::dilate(maskTmp, maskTmp, Mat(), Point(-1, -1), 11);  // 6 .. 10;
                    //cv::dilate(maskTmp, maskTmp, erdKern, Point(-1, -1), 12);  // 6 .. 10;
                    //maskTmp = 0xFF - maskTmp;
                    //if(DEV_MODE >= 4 && extraVis)
                    //    showCvWnd("9.6.0.9.ErdMaskClaheNBg", maskTmp, cvWnds);
                    //edgesOrig -= maskTmp;
...
//                    edgesOrig -= maskClaheAth;  // 9.2.RfnErdCntRfnAthProbFgRoi
                    ...
                    //    showCvWnd("9.6.0.10.CntRfnEdgesOrigRoi", maskHullClaheAth, cvWnds);
                    //}
...
//                        showCvWnd("10.2.MclRfnMopnProbFgRoi", maskClaheAth, cvWnds);
                    ...
                    // Note: pure (true) foreground and background has not been changed from the previous GRabCut call, so their validation is not requried
                    cv::grabCut(imgClr, maskClaheRoi, fgrect, bgdModel, fgdModel, 1, cv::GC_INIT_WITH_MASK);  // GC_INIT_WITH_RECT | ; or cv::GC_EVAL
                    if(DEV_MODE >= 1 && extraVis)
...
//#endif  // QT_DEBUG
            ...
            if(wndName) {
                ...
                //cv::bitwise_not(mask, mask);  // Invert the mask
